...
def test_recipe():
    ref = RecipePreprocessor('install.yaml')

    assert Path('envy').exists(), "Dev env wasn't created"
    assert (Path('envy') / 'bin' / 'python').exists(), "Path to python does exist" 
    assert Path('winklala').exists(), "No winklala folder created"